...
high_val_addresses = r'./data/high_val_addresses.txt'
BECH32_CONST = 1
BECH32M_CONST = 0x2bc830a3
CHARSET = "qpzry9x8gf2tvdw0s3jn54khce6mua7l"

def main_with_profiling(process_amount):
    pr = cProfile.Profile()
    pr.enable()
    main(process_amount)
    pr.disable()
...
def load_main_data(filepath):
    print(f"load_main_data: {filepath}")
    main_seeds = set()
    try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Bitcoin Key Generation Script')
    parser.add_argument('--process_amount', type=int, default=50000000, help='The number of lines to process (default: 50,000,000)')
    args = parser.parse_args()
    print(f"Starting at -> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"process_amount: {args.process_amount}")
    main(args.process_amount)
